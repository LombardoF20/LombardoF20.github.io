...
function createTextMatte() {
  ...
  ctx.fillText(matteCharacter, width * 0.5, height - 32);
...
ParticleSystem.prototype.generate = function() {
  for (let i = 0; i < this.numParticles; i++) {
    ...
    vo.radius = maxSize;
    (vo.speed = Math.random() * maxSpeed * 2 - maxSpeed), (vo.parent = this);
...
Particle.prototype.setPosition = function() {
  ...
  this.y =
    Math.round(Math.random() * linearRows) * ((height - gutter) / linearRows) +
    gutter * 0.5;
  this.locked =
    reportMatte(Math.round(this.x), Math.round(this.y)) === 0 ? false : true;
};
